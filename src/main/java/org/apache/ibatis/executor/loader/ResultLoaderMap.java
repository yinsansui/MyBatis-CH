...
public class ResultLoaderMap {
  ...
  public boolean load(String property) throws SQLException {
    ...
    if (pair != null) {// 如果属性确实没有被加载，则进行加载
      pair.load();
      return true;
    }
    return false;
  ...
  public static class LoadPair implements Serializable {
    ...
    private LoadPair(final String property, MetaObject metaResultObject, ResultLoader resultLoader) {
      ...
      if (metaResultObject != null && metaResultObject.getOriginalObject() instanceof Serializable) {
        ...
        /* @todo May the parameter benull ? */
        if (mappedStatementParameter instanceof Serializable) {// 如果请求参数支持序列化
          // 设置属性值的查询语句的 SQL 对应的 ID
    ...
    public void load(final Object userObject) throws SQLException {
      if (this.metaResultObject == null || this.resultLoader == null) {// 输出结果对象的封装或者输出结果加载器不存在
        if (this.mappedParameter == null) {// 如果 mappedParameter 不存在，那么 SQL 语句也不存在
